...
class EggRatelist extends Base
{
    ...
    public static function addRate($post)
    {
        ...
        $ratewin_set          = 'egg_' . strtolower(trim($post['ratewin_name']));
    ...
    public static function checkRateDuplicate($post)
    {
        ...
        $ratewin_set  = 'egg_' . strtolower($ratewin_name);
    ...
    {
        $exists_tables = exists_egg_odds_table();
        $pan           = self::where([
            'tokenint' => $tokenint,
            'sel'      => 1
        ])->find();
