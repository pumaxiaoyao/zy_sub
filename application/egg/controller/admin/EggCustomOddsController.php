...
class EggCustomOddsController extends AdminBaseController
{
    /**
     * 显示资源列表
     *
    ...
    public function save(Request $request)
    {
        ...
        // 直属上级就是自己
        // 先找自己的定制赔率
        //        $selfCustomOdds = EggCustomOdds::getOddsByTableAndUser($baseTable, $this->user->tokenint);
        //        if ($selfCustomOdds) {
        ...
        //        查找基础赔率表数据
        $selfOdds = EggOdds::getOddsByTable($baseTable);
        //        }
        ...
        // 存在性检查，防止重复添加
        $exists = EggCustomOdds::isExists($baseTable, $tokenint);
        if ($exists) {
            try {

                $customOddsModel       = EggCustomOdds::getOneByTableAndUser($baseTable, $tokenint);
        ...
        $customOddsModel            = new EggCustomOdds();
