...
class Pk10OddsController extends AdminBaseController
{
    /**
     * 赔率列表
     * @return array
    ...
    public function index()
    {
        ...
        $exists_names = exists_pk10_odds_table();
        if (!is_null($user_id)) {
            $tokenint = get_user_tokenint_by_id($user_id);
            $ratelist = Pk10Ratelist::getListByUser($tokenint)->toArray();
        ...
        foreach ($exists_names as $key => $exists_name) {
            ...
            $letter                 = $arr[1];
            $odds                   = Pk10Odds::getAll($exists_name);
            $oddsArr[$key]['name']  = $letter;
            $oddsArr[$key]['table'] = $exists_name;
            $oddsArr[$key]['odds']  = $odds;
            if (isset($tokenint)) {
                $oddsArr[$key]['custom_odds'] = Pk10CustomOdds::getOddsByTableAndUser($exists_name, $tokenint);
            }
        }
    ...
    public function save(Request $request)
    {
        ...
        $letter                              = $arr[1];
        $call_add_pk10_odds_procudure        = 'call add_pk10_odds(:type)';
        $call_add_pk10_odds_procudure_params = [
            'type' => $letter
        ];
        Base::query($call_add_pk10_odds_procudure, $call_add_pk10_odds_procudure_params);
        $exists = Base::execute('show tables like "' . $newTableName . '"');
        if ($exists) {
            $odds = Pk10Odds::name($newTableName)->select();
            return [
                'status' => 200,
    ...
    public function read($id)
    {
        ...
        if (!is_null($user_id)) {
            $tokenint   = get_user_tokenint_by_id($user_id);
            $customOdds = Pk10CustomOdds::getOddsByTableAndUser($tableName, $tokenint);
        }
        ...
        $odds = Pk10Odds::getAll($tableName);
        return [
            'status' => 200,
            'msg'    => 'success',
            'data'   => [
    ...
    public function delete($id)
    {
        ...
        $exist_tables = exists_pk10_odds_table();
        ...
        if (!Base::tableExists($tableName)) {
            Pk10Ratelist::where(['ratewin_set' => $tableName])->delete();
            return [
                'status' => 200,
