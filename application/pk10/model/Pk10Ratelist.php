...
class Pk10Ratelist extends Base
{
    ...
    public static function addRate($post)
    {
        ...
        $ratewin_set          = 'pk10_' . strtolower(trim($post['ratewin_name']));
    ...
    public static function checkRateDuplicate($post)
    {
        ...
        $ratewin_set  = 'pk10_' . strtolower($ratewin_name);
    ...
    public static function updateRate($put, $id)
    {
        ...
        try {
            ...
            $rate               = self::get($id);
            $rate->ratewin_name = strtolower($ratewin_name);
            $rate->ratewin_set  = $ratewin_set;
            $rate->sel          = $put['sel'];
    ...
    {
        $exists_tables = exists_pk10_odds_table();
        $pan           = self::where([
            'tokenint' => $tokenint,
            'sel'      => 1
        ])->find();
