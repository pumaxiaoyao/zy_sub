...
class SscCustomOddsController extends AdminBaseController
{
    /**
     * 显示资源列表
     *
    ...
    public function save(Request $request)
    {
        ...
        // 直属上级就是自己
        // 先找自己的定制赔率
        //        $selfCustomOdds = SscCustomOdds::getOddsByTableAndUser($baseTable, $this->user->tokenint);
        //        if ($selfCustomOdds) {
        ...
        //        查找基础赔率表数据
        $selfOdds = SscOdds::getOddsByTable($baseTable);
        //        }
        ...
        // 存在性检查，防止重复添加
        $exists = SscCustomOdds::isExists($baseTable, $tokenint);
        if ($exists) {
            try {

                $customOddsModel       = SscCustomOdds::getOneByTableAndUser($baseTable, $tokenint);
        ...
        $customOddsModel            = new SscCustomOdds();
